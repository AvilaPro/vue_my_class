...
function mostrarEvent(event) {
  console.log(event);
  alert( "Diste click en: " +event.target.innerHTML);s
}
</script>
...
<template>
  <header @click.self="mostrarInfo('Click en el Header')">
    ...
    <nav id="navHeader">
      <ul id="linksNav">
        <li :class="varPesoFuente" v-on:click.stop="mostrarEvent">{{ inicio.btn1 }}</li>
        <li v-bind="cssLiSubrayado.st" v-on:click.stop="mostrarInfo(inicio.btn2)">{{ inicio.btn2 }}</li>
  ...
  <main>
    ...
    <section id="registerForm" style="padding: 15vh; display: flex;">
      <form style="border: solid 1px black; border-radius: 12px; width: fit-content; padding: 25px;" action="">
        <h1>Registro</h1>
        <p>Nombre escrito: {{ nombreRegistro }}</p>
        Nombre: <input type="text" name="" id="" v-model="nombreRegistro" maxlength="10"><br><br>
        Sexo: 
        <label for="male">Masculino</label>
        <input type="radio" name="sexo" :value="male" v-model="sexoSeleccionado" id="male">
        <label for="female">Femenino</label>
        <input type="radio" name="sexo" :value="female" v-model="sexoSeleccionado" id="female"><br><br>
        <p>Preferencias seleccionadas: <span v-if="prefers.length > 0">{{ prefers }}</span> </p>
        Preferencias:
        <input type="checkbox" name="a" id="pa" value="Arte" v-model="prefers">
        <label for="pa">Arte</label>
        <input type="checkbox" name="c" id="pc" value="Ciencia" v-model="prefers">
...
<style scoped>
header {
  background-image: linear-gradient(to bottom, #ffffff30, #ebebeb47, #d7d7d74a, #c4c4c441, #b1b1b11a);
  height: 70px;
  width: 100vw;
  padding: 0 2%;
  position: fixed;
